...
module MotionPrime
  class FormSection < TableSection
    ...
    def keyboard_will_show
      ...
      current_inset.bottom = KEYBOARD_HEIGHT_PORTRAIT + (self.table_element.options[:bottom_content_inset] || 0)
      table_view.contentInset = current_inset
    end

    def keyboard_will_hide
      current_inset = table_view.contentInset
      current_inset.bottom = self.table_element.options[:bottom_content_inset] || 0
      table_view.contentInset = current_inset
    end
