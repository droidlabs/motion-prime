class TableDataIndexes
  def initialize(data)
    is_flat_data = !data.first.is_a?(Array)
    @count_in_sections = if is_flat_data
      [data.count]
    else
      data.map(&:count)
    end

    @sections_count = is_flat_data ? 1 : data.count
  end
  ...
  def sum_index(a, rows, crop_to_edges = true)
    ...
    if row < 0 || row > max_row
      ...
      start_row = edge_row.zero? ? @count_in_sections[section] - 1 : 0
